...
async function deleteFreelancer(id) {
    try {
        const sqlQuery = 'DELETE FROM Freelancers WHERE FreelancerID = ?';
        await query(sqlQuery, [id]);
        return { message: `Freelancer con ID ${id} eliminado exitosamente` };
    } catch (error) {
        console.error('Error al eliminar freelancer:', error);
        throw error;
    }
}
